...
# # -----------------------------
# from blogChatbotBackend import app
...
def extract_latest_state(current_state: Dict[str, Any], step_payload: Any) -> Dict[str, Any]:
    if isinstance(step_payload, dict):
        # handle {"node_name": {...}} or plain dict updates
        if len(step_payload) == 1 and isinstance(next(iter(step_payload.values())), dict):
            inner = next(iter(step_payload.values()))
            current_state.update(inner)
        else:
    ...
    if not past_files:
        ...
        if st.button("📂 Load selected blog"):
            if selected_md_file:
                ...
                # also update the topic input hint (kept as session hint)
                st.session_state["topic_prefill"] = extract_title_from_md(md_text, selected_md_file.stem)

# Keep topic_prefill as a hint (do not mutate widgets)
if "topic_prefill" in st.session_state and isinstance(st.session_state["topic_prefill"], str):
    pass
...
if run_btn:
    ...
    # Inputs tailored to your backend/state schema (final_blog key used)
    inputs: Dict[str, Any] = {
        "topic": topic.strip(),
        "mode": "",
        "needs_research": False,
...
if out:
    ...
    with tab_preview:
        st.subheader("Markdown Preview")
        final_md = out.get("final_blog") or ""
        if not final_md:
            st.warning("No final markdown found.")
        else:
            # simple rendering (no image features)
